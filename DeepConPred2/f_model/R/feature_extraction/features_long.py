...
import string
import numpy as np
from funs_in_features_long import *
# import time
...
def features(identifier):
	...
	for id1 in range(N-25):  
		for id2 in range(id1+25, N):
			...
			global_list_long.outfile_res_pairs_SS.extend([temp])

			# print ('feature 7 time: {} . \n'.format(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))))

			# feature 7 : residue pairs: solvent accessibility
			# feature dimension : 2 * 13 * 2 = 52
			temp=''
			temp=temp+res_pair+' '
			for i in range(id1-(w-1)/2, id1+(w-1)/2+1):
				if (i >= 0) and (i < N):
			...
			for i in range(id2-(w-1)/2, id2+(w-1)/2+1):
				if (i >= 0) and (i < N):
					...
					temp=temp+'0 0 '
			global_list_long.outfile_res_pairs_ACC.extend([temp])

			# print ('feature 2 time: {} . \n'.format(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))))

			# feature 2 : residue pairs: smoothed PSSM
			# feature dimension : 2 * 13 * 20 = 520
			temp=''
			temp=temp+res_pair+' '
			...
			for i in range(id2-(sw-1)/2, id2+(sw-1)/2+1):
				if (i >= 0) and (i < N):
					...
					for j in range(20):
						temp=temp+'0 '
			global_list_long.outfile_res_pairs_smoothed_PSSM.extend([temp])
			...
			global_list_long.outfile_res_pairs_coevo_info.extend([temp])
			...
			global_list_long.outfile_res_pairs_SSCP.extend([temp])
			...
			for index in range(len(index_seqID)): #index_seqID stores aa indexes of two SSEs
				...
				if (id1 in index_seqID[index]) and (id2 in index_seqID[index]):
					...
					global_list_long.outfile_res_pairs_SSConProb.extend([temp])
					label = True
					break
			if not label:
				temp=temp+'0 0 0 '
			...
			for i in range(rmid-(cw-1)/2, rmid+(cw-1)/2+1):
				if SS_line[i] == 'H':
					...
					temp=temp+('0 0 1 ')
			global_list_long.outfile_intervening_seq_SS.extend([temp])
			...
			global_list_long.outfile_intervening_seq_len.extend([temp])
			...
			global_list_long.outfile_intervening_seq_NV.extend([temp])
			...
			global_list_long.outfile_entire_prot_ACC.extend([temp])
			...
			if N <= 80:
				...
				temp=temp+('0 0 0 1 ')
			global_list_long.outfile_entire_prot_len.extend([temp])
