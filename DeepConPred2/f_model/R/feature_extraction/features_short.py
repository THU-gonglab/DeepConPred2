...
import string
import numpy as np
from funs_in_features_short import *
# import time
...
def features(identifier):
	...
	for id1 in range(N-7):   
		tail= N if id1+13>N else id1+13
		for id2 in range(id1+7, tail):
			...
			global_list_short.outfile_res_pairs_SS.extend([temp])

			# print ('feature 7 time: {} . \n'.format(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))))

			# feature 7 : residue pairs: solvent accessibility
			# feature dimension : 2 * 5 * 2 = 20
			temp=''
			temp=temp+res_pair+' '
			for i in range(id1-(w-1)/2, id1+(w-1)/2+1):
				if (i >= 0) and (i < N):
			...
			for i in range(id2-(w-1)/2, id2+(w-1)/2+1):
				if (i >= 0) and (i < N):
					...
					temp=temp+'0 0 '
			global_list_short.outfile_res_pairs_ACC.extend([temp])
			...
			for i in range(id2-(sw-1)/2, id2+(sw-1)/2+1):
				if (i >= 0) and (i < N):
					...
					for j in range(20):
						temp=temp+'0 '
			global_list_short.outfile_res_pairs_smoothed_PSSM.extend([temp])
			...
			global_list_short.outfile_res_pairs_coevo_info.extend([temp])
			...
			global_list_short.outfile_res_pairs_SSCP.extend([temp])
			...
			for index in range(len(index_seqID)): #index_seqID stores aa indexes of two SSEs
				...
				if (id1 in index_seqID[index]) and (id2 in index_seqID[index]):
					...
					global_list_short.outfile_res_pairs_SSConProb.extend([temp])
					label = True
					break
			if not label:
				temp=temp+'0 0 0 '
			...
			for i in range(mid-(cw-1)/2, mid+(cw-1)/2+1):
				if SS_line[i] == 'H':
					...
					temp=temp+('0 0 1 ')
			global_list_short.outfile_intervening_seq_SS.extend([temp])
			...
			temp=temp+(res_pair+' ')
			if sep == 6:
				temp=temp+('1 0 0 0 0 0 ')
			elif sep==7:
				temp=temp+('0 1 0 0 0 0 ')
			elif sep==8:
				temp=temp+('0 0 1 0 0 0 ')
			elif sep==9:
				temp=temp+('0 0 0 1 0 0 ')
			elif sep==10:
				temp=temp+('0 0 0 0 1 0 ')
			elif sep==11:
				temp=temp+('0 0 0 0 0 1 ')
			
			global_list_short.outfile_intervening_seq_len.extend([temp])
			...
			global_list_short.outfile_intervening_seq_NV.extend([temp])
			...
			global_list_short.outfile_entire_prot_ACC.extend([temp])
			...
			if N <= 80:
				...
				temp=temp+('0 0 0 1 ')
			global_list_short.outfile_entire_prot_len.extend([temp])
